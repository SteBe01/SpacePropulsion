function [masses] = divergent_mass(geom, nozzle, masses)
    ...
    xy_r_down = nozzle.xy_r_up;

    V_bell = pi*(integral(@(x) (f_bell(x)+thick).^2, x1, x2) - integral(@(x) f_bell(x).^2, x1, x2));
    
    V_cone=1/3*(x2-2)*pi*((geom.r_t+thick)^2 + (geom.r_exit+thick)^2 + sqrt((geom.r_t+thick)^2*(geom.r_exit+thick)^2))...
